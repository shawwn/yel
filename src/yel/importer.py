...
class YelLoader (Loader):

    @staticmethod
    def compile_file(filename, outfile) -> str:
        outfile = os.path.abspath(outfile)
        cwd = os.getcwd()
        try:
            os.chdir(os.path.dirname(filename) or ".")
            out = bel.compile_file(os.path.basename(filename))
        finally:
            os.chdir(cwd)
        # bel.write_file(outfile, out)
        return out
    ...
    def exec_module (klass, module):
        ...
        bel.write_file(file_path, source)
...
class YelFinder (MetaPathFinder):
